...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=0.8cm,
        block/.style={draw, rounded corners, align=center, minimum width=3.6cm, minimum height=0.9cm},
        arrow/.style={-Latex, thick}
    ]
        \node[block] (cam) {ESP32-CAM / Webcam\\captura frame};
        \node[block, below=of cam] (stream) {MJPEG HTTP o\\\texttt{VideoCapture(0)}};
        \node[block, below=of stream] (pre) {OpenCV\\lectura y preproceso};
        \node[block, below left=0.8cm and 0.6cm of pre] (yolo) {YOLOv8n ONNX\\personas/vehiculos};
        \node[block, below right=0.8cm and 0.6cm of pre] (ocr) {ROI patente\\binarizacion + OCR};
        \node[block, below=2.1cm of pre] (decision) {Aforo o control\\autorizado/no autorizado};
        \draw[arrow] (cam) -- (stream);
        \draw[arrow] (stream) -- (pre);
        \draw[arrow] (pre) -- (yolo);
        \draw[arrow] (pre) -- (ocr);
        \draw[arrow] (yolo) -- (decision);
        \draw[arrow] (ocr) -- (decision);
    \end{tikzpicture}
    \caption{Diagrama de flujo del procesamiento implementado.}
    \label{fig:flujo}
\end{figure}
...
\begin{table}[H]
    ...
    Repositorio de código & Cumplido & \url{https://github.com/ZharOOX/microprocesadores-Utem}. \\
...
\begin{itemize}
    ...
    \item Repositorio GitHub: \url{https://github.com/ZharOOX/microprocesadores-Utem}.
